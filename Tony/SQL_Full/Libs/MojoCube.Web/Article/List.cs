...
namespace MojoCube.Web.Article
{
    public class List
    {
        #region 公共属性
        int _pk_Article;
        public int pk_Article
        {
            get { return _pk_Article; }
            set { _pk_Article = value; }
        }
        int _CategoryID1;
        ...
        public void GetData(int id)
        {
            ...
            {
                conn.Open();
                string sql = "select * from Article_List where pk_Article=@pk_Article";
                SqlCommand comm = new SqlCommand(sql, conn);
                comm.Parameters.Add(new SqlParameter("@pk_Article", SqlDbType.Int));
                comm.Parameters["@pk_Article"].Value = id;
                SqlDataReader dr = comm.ExecuteReader();
                if (dr.Read())
                {
                    _pk_Article = Convert.ToInt32(dr["pk_Article"].ToString());
        ...
        public void UpdateData(int id)
        {
            ...
            {
                conn.Open();
                string sql = "update Article_List set CategoryID1=@CategoryID1,CategoryID2=@CategoryID2,PageName=@PageName,Title=@Title,Subtitle=@Subtitle,Description=@Description,SEO_Title=@SEO_Title,SEO_Keyword=@SEO_Keyword,SEO_Description=@SEO_Description,Tags=@Tags,Visual=@Visual,Author=@Author,Source=@Source,SourceUrl=@SourceUrl,ImagePath=@ImagePath,Issue=@Issue,IsComment=@IsComment,IsRecommend=@IsRecommend,Clicks=@Clicks,SortID=@SortID,TypeID=@TypeID,Score=@Score,ScoreIn=@ScoreIn,StartDate=@StartDate,EndDate=@EndDate,CreateDate=@CreateDate,CreateUserID=@CreateUserID,ModifyDate=@ModifyDate,ModifyUserID=@ModifyUserID,Language=@Language where pk_Article=@pk_Article";
                SqlCommand comm = new SqlCommand(sql, conn);
                comm.Parameters.Add(new SqlParameter("@CategoryID1", SqlDbType.Int));
                comm.Parameters["@CategoryID1"].Value = _CategoryID1;
                comm.Parameters.Add(new SqlParameter("@CategoryID2", SqlDbType.Int));
        ...
        {
            Sql.SqlQuery("delete from Article_List where pk_Article=" + id);
        }

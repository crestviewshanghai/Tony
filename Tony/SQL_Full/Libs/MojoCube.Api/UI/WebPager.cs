...
namespace MojoCube.Api.UI
{
    public class WebPager
    {
        ...
        public void SetPagerInfo()
        {
            ...
            if (_Language == "zh-cn")
            {
                _Pager.FirstPageText = "<span class=btnLink title='首页'>首页</span>";
                _Pager.LastPageText = "<span class=btnLink title='尾页'>尾页</span>";
                _Pager.NextPageText = "<span class=btnLink title='下一页'>下一页</span>";
                _Pager.NumericButtonTextFormatString = "<span class=Pager>{0}</span>";
                _Pager.PrevPageText = "<span class=btnLink title='上一页'>上一页</span>";
                if (ShowCustomInfo)
                {
            ...
            {
                _Pager.FirstPageText = "<span class=btnLink title='首頁'>首頁</span>";
                _Pager.LastPageText = "<span class=btnLink title='尾頁'>尾頁</span>";
                _Pager.NextPageText = "<span class=btnLink title='下一頁'>下一頁</span>";
                _Pager.NumericButtonTextFormatString = "<span class=Pager>{0}</span>";
                _Pager.PrevPageText = "<span class=btnLink title='上一頁'>上一頁</span>";
                if (ShowCustomInfo)
                {
                    _Pager.CustomInfoHTML = "<div class='cleft'>共<span>" + _Pager.RecordCount + "</span>條 | 每頁<span>" + _Pager.PageSize + "</span>條 | 共<span>" + _Pager.PageCount + "</span>頁</div>";
                }
            }
            else
            {
